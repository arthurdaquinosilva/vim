...
" Only install on builds without a native clipboard (the Linux VM). On macOS
" (+clipboard), yanks already reach the system clipboard via `unnamed`/`unnamedplus`,
" and racing OSC52 against that just clobbers the clipboard with stale data.
if !has('clipboard')
    augroup osc52_yank
        autocmd!
