...
" <leader>tc rebuilds a linked TOC of the H2 headers. The TOC lives between
" <!-- TOC --> / <!-- /TOC --> markers: the first run inserts it after the H1,
" later runs replace it in place, so it's safe to re-run.
...
function s:GenerateTOC() abort
    ...
    " Collect H2 headers, skipping anything inside a ``` fenced code block.
    let l:toc = []
    let l:in_fence = 0
    for l:line in l:lines
        ...
        elseif !l:in_fence && l:line =~# '^##\s\+\S'
            ...
            call add(l:toc, '- [' . l:title . '](#' . s:Slug(l:title) . ')')
        endif
    endfor

    if empty(l:toc)
        echohl WarningMsg | echo 'No H2 headers found' | echohl None
        return
    endif
